...
const MealCountCard = (props) => {
    ...
    return (
        <div className='grid grid-cols-3 font-bold px-12 py-1 mx-12 my-2 bg-[whitesmoke] rounded'>
            <div className='flex items-center'>
                <p>{monthName[month]} {props.date}</p>
            </div>
            <div className='flex items-center'>
                <p className={props.mealCount==0? 'text-red-500' : 'text-green-500'}>{props.mealCount}</p>
            </div>
            <div className='flex items-center'>
                <div>
